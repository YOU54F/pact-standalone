...
For windows x86_64
    ...
    cd windows
    bash -c 'mkdir -p cache output/3.4.7'
    bash -c './build-ruby -a x86 -r 3.4.7 cache output/3.4.7'
    bash -c './package -r traveling-ruby-20230428-3.4.7-x86-windows.tar.gz output/3.4.7'

For windows x86

    bash -c 'mkdir -p cache output/3.4.7'
    bash -c './build-ruby -a x86_64 -r 3.4.7 cache output/3.4.7'
    bash -c './package -r traveling-ruby-20230428-3.4.7-x86_64-windows.tar.gz output/3.4.7'
...
3. Ensure the version number in `tasks/package.rake` matches your package name
   1. eg
      1. `traveling-ruby-20230508-3.4.7-linux-arm64.tar.gz`

    ```ruby
    TRAVELING_RUBY_VERSION = "20230508-3.4.7"
    ```
...
| -------| ------- | ------------ | --------- |
| MacOS    | 3.4.7     | x86_64       | ✅         |
| MacOS    | 3.4.7     | aarch64 (arm)| ✅         |
| Linux  | 3.4.7   | x86_64       | ✅         |
| Linux  | 3.4.7   | aarch64 (arm)| ✅          |
| Windows| 3.4.7 | x86_64       | ✅        |
| Windows| 3.4.7 | x86       | ✅        |
| Windows| 3.4.7 | aarch64 (via x86 emulation) |  ✅        |
